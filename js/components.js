...
def({
  ...
  params: { label: "SRC", power: 0, freq: "1 GHz" },
  fields: [{ key: "power", label: "Output level", unit: "dBm", step: 0.5 }, { key: "freq", label: "Frequency", type: "text" }],
...
def({
  ...
  params: { label: "LO", power: 10, freq: "0.9 GHz" },
  fields: [{ key: "power", label: "Drive level", unit: "dBm", step: 0.5 }, { key: "freq", label: "Frequency", type: "text" }],
  isSource: () => true, srcOut: () => "out", srcPower: p => p.power, val: p => p.freq || "",
  sym() { return `<circle class="blk-shape" cx="20" cy="20" r="19"/><path class="blk-glyph" d="M10 20 q5 -7 10 0 t10 0"/>`; }
});
...
/* --- Terminals ---------------------------------------------------- */
def({
  type: "rfin", keys: "connector input port sma source feed", name: "In connector", group: "Terminals", w: 40, h: 40,
  ports: [{ id: "out", side: "right", kind: "out", dx: 40, dy: 20 }],
  params: { label: "RF IN", power: 0, freq: "" },
  fields: [{ key: "power", label: "Input level", unit: "dBm", step: 0.5 }, { key: "freq", label: "Frequency", type: "text" }],
  isSource: () => true, srcOut: () => "out", srcPower: p => p.power, val: p => p.freq || "input",
  sym() { return `<circle class="blk-shape" cx="13" cy="20" r="9"/><circle class="blk-fillg" cx="13" cy="20" r="3"/><path class="blk-line" d="M22 20H40"/>`; }
});
...
def({
  ...
  params: { label: "RF OUT" }, fields: [], val: () => "output",
...
def({
  ...
  ports: [{ id: "in", side: "left", kind: "in", dx: 0, dy: 20 }],
  params: { label: "DET" }, fields: [], val: () => "video",
...
def({
  ...
  params: { label: "ANT", role: "Tx", power: -80 },
  fields: [{ key: "role", label: "Role", type: "select", options: ["Tx", "Rx"] }, { key: "power", label: "Received level", unit: "dBm", step: 1, showIf: p => p.role === "Rx" }],
...
def({
  ...
  ports: [{ id: "in", side: "left", kind: "in", dx: 0, dy: 20 }],
  params: { label: "LOAD" }, fields: [], val: () => "50 \u03a9",
